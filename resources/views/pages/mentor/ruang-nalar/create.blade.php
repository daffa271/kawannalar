<x-layouts.mentor title="Upload Catatan — KawanNalar">
    <div class="mx-auto max-w-3xl space-y-8">
        <div>
            <p class="text-sm font-bold text-[#0A52C4]">Ruang Nalar</p>
            <h1 class="mt-1 text-2xl font-extrabold text-gray-900 lg:text-3xl">Bagikan Catatan dan Materi</h1>
            <p class="mt-2 text-sm text-gray-500">Materi akan tampil untuk siswa setelah selesai direview admin.</p>
        </div>
        ...
        @if ($errors->any())<div class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">Periksa kembali data modul dan file yang dipilih.</div>@endif
        <form method="POST" action="{{ route('mentor.ruang-nalar.store') }}" enctype="multipart/form-data" class="space-y-6 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-8">
            @csrf
            <div><label for="title" class="field-label">Judul Modul</label><input id="title" name="title" value="{{ old('title') }}" required class="field-input" placeholder="Contoh: Penalaran Matematika Dasar"><x-input-error :messages="$errors->get('title')" class="field-error" /></div>
            <div><label for="description" class="field-label">Deskripsi</label><textarea id="description" name="description" rows="4" class="field-input" placeholder="Jelaskan isi dan manfaat modul...">{{ old('description') }}</textarea><x-input-error :messages="$errors->get('description')" class="field-error" /></div>
